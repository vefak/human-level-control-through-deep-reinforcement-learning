...
def make_atari_deepmind(env_id, max_episode_steps=None, scale_values=False, clip_rewards=True):
    ...
    env = NoopResetEnv(env, noop_max=30) # No operation up to 30

    if 'NoFrameskip' in env.spec.id: # frame skipping
    ...
    env = WarpFrame(env) # resizing to 84x84

    if scale_values:
        env = ScaledFloatFrame(env)

    
    if clip_rewards: # reward clipping 
